      ...
      return (
        <div className={className} style={style}>
          {status === "error_file_size" && (
            <span>
              {meta.size < extra.minSizeBytes
                ? "File too small"
                : "File too big"}
            </span>
          )}
          {status === "error_validation" && (
            <span>{String(meta.validationError)}</span>
          )}
          {canRemove && (
    ...
    return (
      <div className="dzu-previewStatusContainer col py-3">
        <div className="col-6">
          ...
          {!previewUrl && (
            <span className="dzu-previewImage">
              <AiOutlineFileDone size={70}></AiOutlineFileDone>
            </span>
          )}
          <label>{name}</label>
        </div>
      ...
      <div className="col-12">
        <Dropzone
          ...
          inputContent={(files, extra) =>
            extra.reject
              ? "Only Image, obj and mtl are accepted!"
              : "Select and Drop Files"
          }
          styles={{
